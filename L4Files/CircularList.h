...
void CircularList<T>::remove(int index) 
{
   ...
   if (index >= 1 && index <= sze) 
   {
      ...
      else
      {
        ...
        DoubleNode<T>* delNode = find(index);
		
		DoubleNode<T>* prev = delNode->getPrev();
		DoubleNode<T>* next = delNode->getNext();
		next->setPrev(prev);
		prev->setNext(next);
		delNode->setNext(NULL);
		delNode->setPrev(NULL);
...
void CircularList<T>::draw(wxDC& dc, int width, int height)
{
   ...
   while(iter->hasNext())
   {
       ...
       count++;
       CSC1310::String* text = item->getKey();  
       Circle* circle;
       if (count == loc_pos)
       {

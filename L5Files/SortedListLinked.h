...
NextNode<T>** SortedListLinked<T>::find(T* item)
{
   ...
   //type of data inside of NextNode IS item
   //want to return pointer of NextNode
   //NextNode** -> array of pointers  prev, next, curr
   //find function returns an array of pointers

   //DO THIS
   //loop to find the correct location to insert/remove item

   int check, temp;
   check = (*compare_items) (T* item_1, T* item_2);   //check for first item
   ...
   while(check != 1)
   {
       ...
       check = (*compare_items) (T* item_1, T* item_2);   //checks throguh the loop
   }
